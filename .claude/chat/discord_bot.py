...
def build_help_text() -> str:
    ...
    return (
        ...
        "• `/explain topic:<concept>` — explain a concept using your lecture notes\n"
        "• `/quiz` — flashcard Q&A from your latest lecture\n"
        "   (optional: `source:<filename>` to pick a specific note)\n"
        "• `/plan` — 7-day study plan from your schedule + deadlines\n"
        "• `/progress` — weekly study progress and habit summary\n"
        "• `/research topic:<subject>` — synthesise all vault notes on a topic\n"
...
def run_habits_status() -> tuple[str | None, "discord.Embed | None"]:
    """Build the /habits status embed. Returns (error_text | None, embed | None)."""
    ...
    # Category fields
    for cat, pillars in data["categories"].items():
        emoji = data["category_emoji"].get(cat, "•")
        lines = []
        for p in pillars:
    ...
    for d in data["weekly"]:
        ...
        if pct == 100:
            ...
            week_emojis.append("⬜")
    header = "  ".join(weekday_labels)
    row = "  ".join(week_emojis)
    emb.add_field(name="Weekly", value=f"```\n{header}\n{row}\n```", inline=False)

    done = data["done_count"]
    total = data["total"]
    streak = data["current_streak"]
    best = data["best_streak"]
    emb.set_footer(text=f"Completed {done}/{total} today  |  🔥 Streak: {streak}  |  Best: {best}")
...
def run_habits_check(pillar_raw: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        print(f"run_habits_check failed: {exc}", file=sys.stderr)
        return "❌", f"[habits] check error: {type(exc).__name__}"
    if changed:
        return "✅", f"Checked off: **{matched}**"
    return "✅", f"Already done: **{matched}**"
